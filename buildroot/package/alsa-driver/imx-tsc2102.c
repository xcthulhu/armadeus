...
static void audio_playback_stop_dma(audio_stream_t * s)
{
	...
	/*mxc_dma_disable*/imx_dma_disable(s->dma_wchannel);
...
*/
dma_addr_t lsrc_addr, ldst_addr;
int lnum_of_bytes;	
	int device;
	int stream_id;
...
*/
	if (s->active) {
		if (ssi_get_status(s->ssi) & ssi_transmitter_underrun) {
		...
		/*dma_request.*/lsrc_addr = (dma_addr_t) (dma_map_single(NULL,
								    runtime->
								    dma_area +
								    offset,
								    dma_size,
...
/*		if (stream_id == 0) {
			...
			dma_request.*/ldst_addr = (dma_addr_t)(&SSI_STX);
/*			    (dma_addr_t) (SSI1_BASE_ADDR + MXC_SSI1STX0);
		}*/
		/*dma_request.*/lnum_of_bytes = dma_size;
...
/*		mxc_dma_config(s->dma_wchannel, &dma_request, 1,
		...
		while (lnum_of_bytes > 0)
		{
			*(int*)(ldst_addr) = *(int*)(lsrc_addr);
			lsrc_addr +=2;
			lnum_of_bytes -= 2;
		}
...
static int snd_mxc_audio_playback_prepare(struct snd_pcm_substream * substream)
{
	...
	pr_debug(KERN_INFO "MXC audio playback preparing\n");	
	device = substream->pcm->device;
	if (device == 0)
		stream_id = 0;
	else if (device == 1)
...
/*	ssi_interrupt_enable(ssi, ssi_tx_fifo_empty, ssi_Tx);
*/	/*
...
/*!
  ...
  */
static
snd_pcm_uframes_t snd_mxc_audio_capture_pointer(struct snd_pcm_substream * substream)
{
	mxc_pmic_audio_t *chip;
...
*/	else
	...
	msleep(10);
	pr_debug("playback opened device: %d stream_id: %d\n", device , stream_id);
...
*/
	...
	ssi_interrupt_enable(ssi, ssi_rx_fifo_full, ssi_Rx);
					 ...
					 int device)
{
	...
	pr_debug(KERN_INFO "MXC audio pcm support starting\n");	
	/*
	...
	pr_debug(KERN_INFO "MXC audio support starting 2nd device\n");	
...
static int /*__init*/ mxc_alsa_audio_probe(struct platform_device *dev)
{
	...
	pr_debug(KERN_INFO "MXC audio support probing\n");	
	...
	pr_debug(KERN_INFO "MXC audio support starting audio pcm\n");	
	mxc_audio->card = card;
	card->dev = &dev->dev;
