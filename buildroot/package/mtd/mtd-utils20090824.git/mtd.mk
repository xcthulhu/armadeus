...
$(MTD_HOST_DIR)/.unpacked: $(DL_DIR)/$(MTD_SOURCE)
	...
	toolchain/patch-kernel.sh $(MTD_HOST_DIR) \
		package/mtd/mtd-utils20090824.git mtd-utils-all\*.patch
	toolchain/patch-kernel.sh $(MTD_HOST_DIR) \
		package/mtd/mtd-utils20090824.git mtd-utils-host\*.patch
	touch $@


$(MTD_HOST_TARGETS): $(MTD_HOST_DIR)/.unpacked
	CC="$(HOSTCC)" CROSS= CFLAGS=-I$(LINUX_HEADERS_DIR)/include \
		$(MAKE) \
		BUILDDIR=$(MTD_HOST_DIR) WITHOUT_XATTR=1 \
		-C $(MTD_HOST_DIR) 

mtd-host: $(MTD_HOST_TARGETS)
...
$(MTD_DIR)/.unpacked: $(DL_DIR)/$(MTD_SOURCE)
	...
	toolchain/patch-kernel.sh $(MTD_DIR) package/mtd/mtd-utils20090824.git mtd-utils-all\*.patch
	toolchain/patch-kernel.sh $(MTD_DIR) package/mtd/mtd-utils20090824.git mtd-utils-target\*.patch
	touch $@
...
MTD_BUILD_TARGETS_JFFS2 := $(addprefix $(MTD_DIR)/, $(MTD_TARGETS_y))
MTD_BUILD_TARGETS_UBI := $(addprefix $(MTD_DIR)/ubi-utils/, $(MTD_TARGETS_UBI_y))
MTD_BUILD_TARGETS := $(MTD_BUILD_TARGETS_JFFS2) $(MTD_BUILD_TARGETS_UBI)
...
$(MTD_BUILD_TARGETS): $(MTD_DIR)/.unpacked
	...
	$(MAKE1) OPTFLAGS="-DNEED_BCOPY -Dbcmp=memcmp -I$(STAGING_DIR)/usr/include $(TARGET_CFLAGS)" \
		LDFLAGS="$(TARGET_LDFLAGS)" \
	       ...
	       $(MTD_BUILD_TARGETS_JFFS2)

	$(MAKE1) OPTFLAGS="-DNEED_BCOPY -Dbcmp=memcmp -I$(STAGING_DIR)/usr/include $(TARGET_CFLAGS)" \
		LDFLAGS="$(TARGET_LDFLAGS)" \
	       BUILDDIR=$(MTD_DIR)/ubi-utils \
	       CROSS=$(TARGET_CROSS) CC=$(TARGET_CC) WITHOUT_XATTR=1 -C $(MTD_DIR)/ubi-utils \
	       $(MTD_BUILD_TARGETS_UBI)
